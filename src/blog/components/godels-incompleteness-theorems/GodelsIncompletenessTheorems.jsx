  ...
  render () {
    return (
      <Blog title='Gödel’s Incompleteness Theorems'>
        <p>
          Gödel’s proof is applied to some calculus strong enough to describe arithmetic. In his original paper, Gödel used an adaptation of <i>Principia Mathematica</i>. The system I will be assuming is Hofstadter’s Typographical Number Theory (TNT).<sup>1</sup> TNT contains 20 symbols, 5 axioms, 8 rules of inference, and it can codify all of arithmetic. For example, the phrase “<$ $='\text{$x$ is an even number}' />” in TNT could be “<$ $={`\\exists a'' : (\\text{SS}0 \\cdot a'') = a'`} />” where <$ $='x' /> is represented by the free variable <$ $={'a\''} />.
        </p>
        <p>
        ...
        <p>
          TNT is now equipped to do some meta-mathematical introspection. The phrase “<$ $='\text{$T$ is a theorem in TNT}' />” is logically equivalent to the TNT-statement “<$ $={'\\exists a : \\operatorname{Prf}_{\\text{TNT}}\\langle a, \\operatorname{GN}(T) \\rangle'} />.”
        </p>
        <p>
        ...
        <p>
          This corresponds to replacing the free variable “<$ $='a' />” with the numeral “<$ $='\text{S}0' />” to obtain “<$ $='\text{S}0 = \text{S}0' />.” This is again an arithmetical function describable by TNT.<sup>5</sup>
        </p>
        <p>
        ...
        <p>
          This new statement called <$ $='G' /> has a Gödel number, and this number is equal to <$ $='\operatorname{Sub}(n, 2^{12}, n)' />. Furthermore, <$ $='G' /> is a statement in the calculus of TNT that represents the meta-mathematical statement “<$ $='\text{The statement whose Gödel number is $\operatorname{Sub}(n, 2^{12}, n)$ is not a theorem in TNT}' />.” In other words, “<$ $='\text{$G$ is not provable}' />.” Gödel further proved that <$ $='G' /> is provable if and only if its formal negation <$ $='{\sim}G' /> is provable.<sup>6</sup> Since TNT is a consistent system,<sup>7</sup> neither <$ $='G' /> nor <$ $='{\sim}G' /> are provable. By meta-mathematical reasoning, this implies that <$ $='G' />, which asserts a definite numerical property of the natural numbers, is true. <$ $='G' /> is a true TNT-statement that cannot be proven in TNT.
        </p>
        <hr />
        <small>
          <ol>
            ...
            <li>
              Proving consistency of a system usually involves finding a formula in that system that cannot be proven, since if a system were inconsistent, then all formulas could be proven. In TNT, all axioms are tautological, and all rules of inference preserve tautologicalness. Thus the formula “<$ $={'a = a\''} />” which is not a tautology, cannot be proven.
            </li>
          </ol>
        </small>
      </Blog>
