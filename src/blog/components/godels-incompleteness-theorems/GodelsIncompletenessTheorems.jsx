  ...
  render () {
    ...
    return (
      <>
        ...
        <$ $$='\ulcorner\varphi\urcorner = 2^{c_1} \times 3^{c_2} \times 5^{c_3} \times \cdots \times p_k^{c_k} = \prod_{i=1}^k p_i^{c_i}' />
        <p>
          where <$ $='p_i' /> indicates the <$ $='i' /><sup>th</sup> prime number.{createFootnote('gnn')} Thus the symbol <span className={styles.statement}><$ $='\sim' /></span> has Gödel number <$ $='2^{17} = 131,072' />. The statement <span className={styles.statement}><$ $='a = a' /></span> has Gödel number <$ $='2^{12} \cdot 3^{3} \cdot 5^{12} = 2.7 \times 10^{13}' />. Similarly, proofs, which are just sequences of statements, have unique Gödel numbers constructed the same way.{createFootnote('gnu', 'gnp')}
        </p>
        <p>
        ...
        <p>
          One last piece of machinery is required to complete Gödel’s proof. Suppose <$ $='e' /> is a TNT-statement with one free variable <$ $='a' /> and has Gödel number <$ $={'a\''} />. The process of replacing all free variables with a specified constant <$ $={'a\'\''} /> results in a new statement whose Gödel number is denoted as
        </p>
        <$ $$={'\\operatorname{Sub}(a\', \\ulcorner a \\urcorner, a\'\').'} />
        <p>
        ...
        <p>
          This corresponds to replacing the free variable <span className={styles.statement}><$ $='a' /></span> with the numeral <span className={styles.statement}><$ $='\text{S}0' /></span> to obtain <span className={styles.statement}><$ $='\text{S}0 = \text{S}0' />.</span> This is again an arithmetical function describable by TNT.{createFootnote('sba')}
        </p>
        <p>
        ...
        <p>
          This new statement called <$ $='G' /> has a Gödel number, and this number is equal to <$ $='\operatorname{Sub}(n, 2^{12}, n)' />. Furthermore, <$ $='G' /> is a statement in the system of TNT that represents the meta-mathematical statement <span className={styles.statement}>The statement whose Gödel number is <$ $='\operatorname{Sub}(n, 2^{12}, n)' /> is not a theorem in TNT.</span> In other words, <span className={styles.statement}><$ $='\text{$G$ is not provable}' />.</span> Gödel further proved that <$ $='G' /> is provable if and only if its formal negation <$ $='{\sim}G' /> is provable.{createFootnote('gng')} Since TNT is a consistent system,{createFootnote('icp')} neither <$ $='G' /> nor <$ $='{\sim}G' /> are provable. By meta-mathematical reasoning, this implies that <$ $='G' />, which asserts a definite numerical property of the natural numbers, is true. <$ $='G' /> is a true TNT-statement that cannot be proven in TNT.
        </p>
        <p>
        ...
        <hr />
        {displayFootnotes({
          tnt: <p>The exact system is called <i>austere</i> TNT and is described in full in Chapter 8 of <i>Gödel, Escher, Bach: an Eternal Golden Braid</i>.</p>,
          gnn: <p>For a system of <$ $='n' /> symbols, it’s common to just assign each symbol a single base <$ $='n' /> numeral. Then for any string of symbols, replace each by its corresponding base <$ $='n' /> numeral, and the resulting base <$ $='n' /> number is the Gödel number for that string. The numeral <$ $='0' /> must be assigned to a symbol that can never appear first in a valid sentence of the system. For TNT, Hofstadter assigns each symbol a 3-digit “codon,” and Gödel numbers are constructed by concatenating codons into a single base 10 number.</p>,
          gnu: <p>Every Gödel number is unique due to the fundamental theorem of arithmetic.</p>,
          gnp: <p>To make the Gödel number of a proof, an additional 21<sup>st</sup> symbol is needed to act as a punctuation mark delimiting each statement or line of a proof. Alternatively, the standard method is translate the sequence of statements into a sequence of their Gödel numbers <$ $='g_1, \ldots, g_k' />, and let the “super Gödel number” of the sequence to be <$ $='\prod_{i=1}^k p_i^{g_i}' />.</p>,
          ppa: <p>The process of checking if two Gödel numbers form a “proof-pair” involves 1) translating both numbers to their TNT-counterparts, 2) verifying that the last statement of the proof matches the theorem to be proved, and 3) verifying that each statement in the proof is either an axiom or can be obtained by combining previous lines through valid rules. This algorithm is primitive recursive and is therefore provably representable in TNT.</p>,
          sba: <p>The exact formulation of this function is extremely complex and is done in Gödel’s papers. Page 81 of <i>Gödel’s Proof</i> offers a brief attempt.</p>,
          gng: <p>This statement is a simpler, but stronger assertion than what Gödel actually proved. Gödel showed that if <$ $='G' /> is provable, then <$ $='{\sim}G' /> is provable; and if <$ $='{\sim}G' /> is provable, then arithmetic is <$ $='\omega' />-inconsistent.</p>,
          icp: <p>Proving consistency of a system usually involves finding a formula in that system that cannot be proven, since if a system were inconsistent, then all formulas could be proven. In TNT, all axioms are tautological, and all rules of inference preserve tautologicalness. Thus the formula <span className={styles.statement}><$ $={'a = a\''} /></span> which is not a tautology, cannot be proven.</p>
        })}
      </>
    )
  }
